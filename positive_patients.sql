...
select * from ADMISSIONS where SUBJECT_ID=9436 order by ADMITTIME


select DRG_CODE from ADMISSIONS A
    inner join PATIENTS P on A.SUBJECT_ID = P.SUBJECT_ID
    inner join DRGCODES D on A.HADM_ID = D.HADM_ID
where A.HADM_ID = 100834
...
select count(*) from CHARTEVENTS where HADM_ID=115545